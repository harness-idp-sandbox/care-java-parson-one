...
# watch rollout
kubectl rollout status deploy/java-parson-one -n care

# get external endpoint
kubectl get svc java-parson-one-svc -n care
```
...
```bash
kubectl port-forward svc/java-parson-one-svc 8080:80 -n care
...
kubectl rollout restart deploy/java-parson-one -n care
kubectl rollout status deploy/java-parson-one -n care
```
